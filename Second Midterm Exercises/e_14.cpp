...
protected:
    ...
    char broj[20];
    static float DANOK;
    static float DANOK_300;
    static float DANOK_150;
public:

    SMS() {}

    SMS(char * broj = "", float osnovnaCena = 0) {
...
float SMS::DANOK = 18;
float SMS::DANOK_300 = 300;
float SMS::DANOK_150 = 150;
...
public:
    ...
    RegularSMS(char * broj ="", float osnovnaCena = 0, char * message = "", bool roaming=false) : SMS(broj,osnovnaCena) {
    ...
    float SMS_cena() {
        ...
        if(strlen(message)%160==0) {
            brojac = strlen(message)/160;
        }
        else if(strlen(message)%160 !=0) {
            brojac=strlen(message)/160;
            brojac++;
        }
        if(roaming) {
            return  brojac*(osnovnaCena*DANOK_300/100+osnovnaCena);
        }
        else {
            return brojac*(osnovnaCena*DANOK/100+osnovnaCena);
        }
...
public:
    ...
    SpecialSMS(char * broj = "", float osnovnaCena = 0, bool humanitarni =  false) : SMS(broj,osnovnaCena) {
    ...
    float SMS_cena() {
        ...
        return osnovnaCena*DANOK_150/100+osnovnaCena;
    }
...
void vkupno_SMS(SMS**poraka, int n) {
    ...
    for(int i=0; i<n; i++) {
        ...
        if(temp) {
            brojacRegular++;
            maxRegular+=poraka[i]->SMS_cena();
        }
        else {
            brojacSpecial++;
            maxSpecial+=poraka[i]->SMS_cena();
        }
    }
    cout<<"Vkupno ima "<<brojacRegular<<" regularni SMS poraki i nivnata cena e: "<<maxRegular<<endl;
     cout<<"Vkupno ima "<<brojacSpecial<<" specijalni SMS poraki i nivnata cena e: "<<maxSpecial<<endl;
