...
//            totalgrades = totalgrades + ocenki[i];
//        }
//        return totalgrades/brojOcenki;
//    }
...
// PROVERI DETALNO
    virtual int getBodovi() {
        ...
        double avg = (1.0 * passingGrades) / brojOcenki;
        int bodovi = avg*100;
...
public:
    ...
    Demonstrator(int indeks, int *ocenki, int brojOcenki, char *imeIPrezime, Kurs *kursevi, int brojKursevi,
                 int brCasovi) : Student(indeks, ocenki, brojOcenki), Predavach(imeIPrezime, kursevi, brojKursevi) {
    ...
    int getBodovi() {
        ...
        try {
            ...
            bodovi += 20*brCasovi/brojKursevi;
        }
...
Student& vratiNajdobroRangiran(Student ** studenti, int n )
{
    ...
    int bestindex = 0;
    for (int i = 0; i < n; ++i) {
        if(najdobarrank<studenti[i]->getBodovi())
        {
            bestindex = i;
            najdobarrank = studenti[i]->getBodovi();
        }
    }
    return *studenti[bestindex];
}
...
void pecatiDemonstratoriKurs(char* kurs, Student** studenti, int n)
{
    for (int i = 0; i < n; ++i)
    {
        if (Demonstrator* d = dynamic_cast<Demonstrator*>(studenti[i]))
            for (int j = 0; j < d->getBrojKursevi(); j++)
            {
                if (strcmp(d->operator[](j).getIme(), kurs) == 0)
                {
                    d->pecati();
                    cout << endl;
                    break; 
