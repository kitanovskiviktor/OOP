...
public:
    ...
    bool operator > (Oglas &oglas) {
        return cena > oglas.cena;
    }
...
public:
    ...
    Oglasnik(char * ime, Oglas * oglasi, int count) {
        strcpy(this->ime, ime);
        this->count = count;
        this->oglasi = new Oglas[count+1];
    ...
    Oglasnik&operator=(const Oglasnik&other) {
        ...
        this->oglasi = new Oglas[other.count+1];
    ...
    friend ostream&operator<<(ostream &out, Oglasnik &oglasnik) {
        out<<oglasnik.ime<<endl;
        for(int i=0; i<oglasnik.count; i++) {
            out<<oglasnik.oglasi[i]<<endl;
        }
        return out;
...
void najniskaCena() {
    Oglas max = oglasi[0];
    int maxPrice = max.getCena();

    for(int i=0; i<count; i++) {
        if(oglasi[i].getCena() < maxPrice) {
            max = oglasi[i];
            maxPrice = oglasi[i].getCena();
        }
    }
